...
unsigned char tpstate(void)
{
	return 	(*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(0))) + ((4)<<2))));
}
...
unsigned int ReadFromCharFrom7843()             
{
	...
	{
		Num<<=1;		
		(*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(1))) + ((4)<<2))))=1; __nop();__nop();__nop();                
		(*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(1))) + ((4)<<2))))=0; __nop();__nop();__nop();
		if((*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(0))) + ((2)<<2)))))
		{
			Num++;
		}
	}
...
unsigned char Read_ADS2(unsigned int *x,unsigned int *y) 
{
    ...
    flag=Read_ADS(&x2,&y2);	
    if(flag==0)return(0);   
    if(((x2<=x1&&x1<x2+20)||(x1<=x2&&x2<x1+20))
    &&((y2<=y1&&y1<y2+20)||(y1<=y2&&y2<y1+20)))
    {
        *x=(x1+x2)>>1;
        *y=(y1+y2)>>1;		
        return 1;
...
{		 	 
	...
	{
		l=1;
		tp_pixlcd.x=tp_pixad.x>chx?((unsigned long int)tp_pixad.x-(unsigned long int)chx)*1000/vx:((unsigned long int)chx-(unsigned long int)tp_pixad.x)*1000/vx;
		tp_pixlcd.y=tp_pixad.y>chy?((unsigned long int)tp_pixad.y-(unsigned long int)chy)*1000/vy:((unsigned long int)chy-(unsigned long int)tp_pixad.y)*1000/vy;
	}
	return l;
}	   
...
{	
	...
	LCD_ShowString(5,40,"Please adjustment the screen.",0xF800,0xFFFF,16,0);
	TP_Drow_Touch_Point(20,20,0xF800);
	while(1)
	{
		if((*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(0))) + ((4)<<2))))==0)
		{
			if(Read_TP_Once())
			{  								   
				p[cnt].x=tp_pixad.x;
			...
			{			   
				case 1:
					while(!(*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(0))) + ((4)<<2)))))  
					{
					}
					TP_Drow_Touch_Point(20,20,0xFFFF);			
					TP_Drow_Touch_Point(240-21,20,0xF800);	
					break;
				case 2:
					while(!(*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(0))) + ((4)<<2)))))  
					{
					}
 					TP_Drow_Touch_Point(240-21,20,0xFFFF);
					TP_Drow_Touch_Point(20,320-21,0xF800);	 
					break;
				case 3:
					while(!(*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(0))) + ((4)<<2)))))  
					{
					}
 					TP_Drow_Touch_Point(20,320-21,0xFFFF);			
 					TP_Drow_Touch_Point(240-21,320-21,0xF800);	
					break;
				case 4:	 
				  ...
				  while(!(*((volatile uint32_t *)(((((( uint32_t)0x50000000) + 0x4000) + 0x0200)+(0x20*(0))) + ((4)<<2)))))  
					{
			   		...
			   		vx1=p[1].x>p[0].x?(p[1].x-p[0].x+1)*1000/(240-20-20):(p[0].x-p[1].x-1)*1000/(240-20-20);
				 	chx1=p[1].x>p[0].x?p[0].x-(vx1*20)/1000:p[0].x+(vx1*20)/1000;
				   	vy1=p[2].y>p[0].y?(p[2].y-p[0].y-1)*1000/(320-20-20):(p[0].y-p[2].y-1)*1000/(320-20-20);
					chy1=p[2].y>p[0].y?p[0].y-(vy1*20)/1000:p[0].y+(vy1*20)/1000; 
					
					vx2=p[3].x>p[2].x?(p[3].x-p[2].x+1)*1000/(240-20-20):(p[2].x-p[3].x-1)*1000/(240-20-20);
					chx2=p[3].x>p[2].x?p[2].x-(vx2*20)/1000:p[2].x+(vx2*20)/1000;
				   	vy2=p[3].y>p[1].y?(p[3].y-p[1].y-1)*1000/(320-20-20):(p[1].y-p[3].y-1)*1000/(320-20-20);
					chy2=p[3].y>p[1].y?p[1].y-(vy2*20)/1000:p[1].y+(vy2*20)/1000; 


					if((vx1>vx2&&vx1>vx2+1000)||(vx1<vx2&&vx1<vx2-1000)||(vy1>vy2&&vy1>vy2+1000)||(vy1<vy2&&vy1<vy2-1000))
					{
						cnt=0;
						TP_Drow_Touch_Point(240-21,320-21,0xFFFF);	
						TP_Drow_Touch_Point(20,20,0xF800);
						LCD_ShowString(5,40,"Touch Adjust Filed!          ",0xF800,0xFFFF,16,0);
						LCD_ShowString(5,60,"Please Adjust Again!         ",0xF800,0xFFFF,16,0);
						continue;
					}
					vx=(vx1+vx2)/2;
					vy=(vy1+vy2)/2;
					...
					TP_Drow_Touch_Point(240-21,320-21,0xFFFF);	
					LCD_ShowString(5,40,"Touch Screen Adjust OK!      ",0xF800,0xFFFF,16,0);
					LCD_ShowString(5,60,"                             ",0xF800,0xFFFF,16,0);
					delay_ms(1000);
 					LCD_ShowString(5,40,"                       ",0xF800,0xFFFF,16,0);
					return;
			}
		}
	} 
...
unsigned char TP_Init(void)
{
	...
	}else
	{
		...
		LCD_ShowString(10,320-140,fac,0xF800,0xFFFF,16,0);
		sprintf((char*)fac,"vy:%d",vy);
		LCD_ShowString(10,320-120,fac,0xF800,0xFFFF,16,0);
		sprintf((char*)fac,"chx:%d",chx);
		LCD_ShowString(10,320-100,fac,0xF800,0xFFFF,16,0);
		sprintf((char*)fac,"chy:%d",chy);
		LCD_ShowString(10,320-80,fac,0xF800,0xFFFF,16,0);
		return 1;
	}
}
